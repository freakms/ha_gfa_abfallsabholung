...
class GFADataCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, ics_url: str) -> None:
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(self.ics_url, timeout=30) as response:
                    if response.status != 200:
                        raise UpdateFailed(f"Error fetching calendar: HTTP {response.status}")
            ...
            raise UpdateFailed(f"Error communicating with calendar: {err}") from err
        except Exception as err:
            raise UpdateFailed(f"Error parsing calendar: {err}") from err
